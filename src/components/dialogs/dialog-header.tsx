...
export const CustomModalHeader = ({ icon, title, description, className }: CustomModalHeaderProps) => {
  return (
    <DialogHeader className={cn('relative overflow-hidden', className)}>
      <div className="absolute inset-0 bg-gradient-to-r from-blue-50 via-purple-50 to-indigo-50 opacity-50" />
      <div className="relative space-y-1 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <DialogTitle className="text-xl font-semibold leading-tight tracking-tight text-gray-900">
              {title}
            </DialogTitle>
            {description && (
              <DialogDescription className="mt-0.5 text-sm leading-snug text-gray-600">{description}</DialogDescription>
            )}
          </div>
        </div>
      </div>
